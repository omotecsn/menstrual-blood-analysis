...
# Set up Gradio interface for page 1 (Blood Shade Detection)
def create_interface_page_1():
    input_image = gr.Image(type="filepath", label="Upload Image")
    output_image = gr.Image(label="Annotated Image")
    output_text = gr.Textbox(label="Detected Blood Shades and Areas", interactive=False, lines=10)
    total_area_output = gr.Textbox(label="Total Detected Area (px)", interactive=False)

    return gr.Interface(
        fn=process_image,  # Processing function for blood shade detection
        inputs=input_image,
        outputs=[
            output_image, 
            gr.Image(label="Shade 1"), 
            gr.Image(label="Shade 2"), 
            gr.Image(label="Shade 3"), 
            gr.Image(label="Shade 4"), 
            gr.Image(label="Shade 5"), 
            output_text,  # Add output text for detected areas
            total_area_output  # Add output for total area
        ],
        title="Automated Disease Detection",
        description="Upload an image to detect blood shades."
    )
...
def load_images_from_folder(folder_path):
    ...
    return images
# Set up Gradio interface for page 2 (Display Images from Folder)
# Set up Gradio interface for page 2 (Display Images from Folder)
def create_interface_page_2():
    ...
    def display_images_and_description(selected_number):
        ...
        descriptions = {
            1: "16ml volume , medium viscosity ,Area is 14390: Black blood sign of old blood ,might indicate vaginal infection.",
            2: "2ml volume , medium viscosity , Low flow: Area is 3271.5: Black blood sign of old blood, might indicate vaginal infection.",
            3: "16ml volume , low viscosity , 26103.5: Blood red, medium flow, healthy.",
            4: "20 ml volume , low viscosity ,25476: Blood red, healthy ,medium flow.",
            5: "6ml volume , low viscosity , Low flow: 17532 area, healthy.",
            6: "13 ml volume , medium viscosity, 14103.5 area: Black blood sign of old blood, might indicate vaginal infection.",
            7: "16ml volume , high viscosity , 14322 area: Low flow, healthy.",
            8: "16ml volume , low viscosity,High flow: 45689 area, healthy.",
            9: "20 ml volume , medium viscosity , 29910 area: Medium to high flow, healthy.",
            10: "2ml volume, medoum viscosity , Low flow: 6339.0 area, healthy.",
            11: "12ml volume , medium viscosity, Low flow: 14371.5 area, healthy.",
            12: "20ml volume, medium viscosity ,17754 area: Medium flow, healthy.",
            13: "20ml volume , medium viscosity, 24501 area: Healthy ,high flow."
        }
...
def main():
    ...
    blood_shade_interface = create_interface_page_1()
    folder_image_interface = create_interface_page_2()
    
    # Create Gradio tabs to toggle between both functionalities
    demo = gr.TabbedInterface([blood_shade_interface, folder_image_interface], ["Blood Shade Detection", "Pre-clicked photos"])
    demo.launch()
